...
# Преобразуем support_id в список целых чисел для поддержки нескольких администраторов
support_ids = [int(id.strip()) for id in support_id.split(',')]

def panel(message):
    if message.from_user.id in support_ids:  # Проверка, является ли пользователь администратором
...
def new_user_notification(username):
    for admin_id in support_ids:  # Уведомляем всех администраторов
        bot.send_message(admin_id, text=notification.format(username=username))
...
def spam(message, orig_msg):
    ...
    if text.startswith('-'):
        bot.edit_message_text(chat_id=message.chat.id, message_id=orig_msg.message_id, text=canceled_action,
                              reply_markup=markup)
    else:
        info = get_all_user_ids(text)
        bot.edit_message_text(chat_id=message.chat.id, message_id=orig_msg.message_id, text=sender_started)
        success_sent = 0
        ...
        bot.edit_message_text(chat_id=message.chat.id, message_id=orig_msg.message_id,
...
def ban_user_adm(message, orig_msg):
    ...
    if "-" in text:
        ...
        try:
            ...
            print(ex)
            bot.edit_message_text(chat_id=message.chat.id, message_id=orig_msg.message_id, text=error_text)
...
def get_info_user(message, orig_msg):
    ...
    if not info[0]:
        bot.edit_message_text(chat_id=message.chat.id, message_id=orig_msg.message_id, text=error_text, reply_markup=markup)
        return
    ...
    """
    bot.edit_message_text(chat_id=message.chat.id, message_id=orig_msg.message_id, text=information, reply_markup=markup)
